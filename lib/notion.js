...
function ensureNotionConfig() {
  ...
  if (!process.env.NOTION_VIP_PROFILES_DB) missing.push('NOTION_VIP_PROFILES_DB');
  if (!process.env.NOTION_CUSTOMERS_DB) missing.push('NOTION_CUSTOMERS_DB');
  if (missing.length) {
...
export async function getVIPProfile(profileId) {
  ...
  try {
    ...
    // If Notion returns a validation_error about property type mismatch, fall back to scanning pages
    if (e?.code === 'validation_error' && /does not match the property type/i.test(e.message || '')) {
      try {
        ...
        while (true) {
          ...
          for (const item of res.results) {
            ...
            const val = (
              extract.rich_text(p[VIP.PROFILE_ID]) ||
              extract.title(p[VIP.PROFILE_ID]) ||
              (extract.formula(p[VIP.PROFILE_ID]) ?? '') ||
              (extract.number(p[VIP.PROFILE_ID])?.toString()) ||
              ''
            ).toString().toUpperCase();
            if (val === profileId.toUpperCase()) {
              return {
                id: item.id,
                profileId: extract.rich_text(p[VIP.PROFILE_ID]) || extract.title(p[VIP.PROFILE_ID]) || extract.formula(p[VIP.PROFILE_ID]) || extract.number(p[VIP.PROFILE_ID]) || '',
                name: extract.title(p[VIP.NAME]),
                password: extract.rich_text(p[VIP.PASSWORD]),
                phone: extract.phone(p[VIP.PHONE]),
                email: extract.email(p[VIP.EMAIL]),
        ...
        console.error('getVIPProfile fallback scan error:', inner);
        throw inner;
      }
    }
