...
const extract = {
  ...
  // status type (different from select!)
  status: (p) => p?.status?.name || '',
  // multi_select returns array of names
  multi_select: (p) => p?.multi_select?.map(s => s.name).join(', ') || '',
  url: (p) => p?.url || '',
  email: (p) => p?.email || '',
...
const VIP = {
  ...
  PORTAL_FOLDER: 'Portal Folder',  // url - direct link to portal folder in Drive
};

// Customer property names (EXACT match from Notion database!)
const CUST = {
  DOC_NAME: 'Name',                    // title field (document name)
  CUSTOMER_ID: 'Customer ID',          // unique_id or formula
  LANGUAGE: 'Language 🚩',             // multi_select
  PAGES: 'Page',                       // number (single page count)
  WORDS: 'Words',                      // number
  STATUS: 'Status',                    // status type
  PAYMENT: 'Payment Status 💰',        // status type
  TOTAL: 'Final Quotation',            // number (final price)
  FOLDER: 'Ticket Folder',             // url
  PDF_FOLDER: 'PDF Folder',            // url
  VIP_PROFILE: 'Customer Profile ',    // relation (with trailing space!)
};
  ...
  try {
    ...
    if (res.results?.length) {
      ...
      return {
        ...
        portalFolder: extract.url(p[VIP.PORTAL_FOLDER]),
...
export async function getProjects(vipPageId, options = {}) {
  ...
  try {
    ...
    return res.results.map(page => {
      ...
      return {
        id: page.id,
        customerId: extract.formula(p[CUST.CUSTOMER_ID]) || extract.unique_id(p[CUST.CUSTOMER_ID]) || page.id.slice(0, 8),
        documentName: extract.title(p[CUST.DOC_NAME]),
        language: extract.multi_select(p[CUST.LANGUAGE]),  // multi_select, not select
        pages: extract.number(p[CUST.PAGES]),
        words: extract.number(p[CUST.WORDS]),
        status: extract.status(p[CUST.STATUS]),            // status type, not select
        paymentStatus: extract.status(p[CUST.PAYMENT]),    // status type, not select
...
export async function getStats(vipPageId) {
  ...
  // Status values from Notion: "Archive", "Done", etc.
  ...
  return projects.reduce((s, p) => ({
    ...
    activeProjects: s.activeProjects + (!completedStatuses.includes(p.status) ? 1 : 0),
    completedProjects: s.completedProjects + (completedStatuses.includes(p.status) ? 1 : 0),
